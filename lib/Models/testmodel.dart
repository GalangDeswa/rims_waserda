...
class ProductTile extends GetView<kasirController> {
  ...
  Widget build(BuildContext context) {
    return DataTable(
      ...
      rows: controller.productlist
          .map((produktable) => DataRow(cells: [
                DataCell(
                  Text(produktable.name),
                  /*onTap: () {
                          print('Selected ${txn.date}');
                        }*/
                ),
                DataCell(FittedBox(
                  child: Text(produktable.price),
                )),
                DataCell(
                  Text(produktable.category),
                )
              ]))
          .toList(),
    );
...
class ProductTilev3 extends GetView<kasirController> {
  final String lol;
  ProductTilev3(this.lol);

  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text(controller.isitest
...
class rowlist extends GetView<kasirController> {
  final Product product;

  const rowlist(this.product);
